        ...
        > 
          ...
          <div className="grid grid-cols-9 grid-rows-9 gap-0.5 w-full h-full bg-gray-800 p-1 rounded-lg">
            ...
            {boardTiles.map((tile, i) => {
              ...
              return (
                <div key={tile.id} onClick={() => handleTileClick(i)} 
                  ...
                  <div className="absolute flex gap-0.5 z-20 pointer-events-none">
            ...
            <div className="bg-white col-start-2 col-end-9 row-start-2 row-end-9 m-0.5 rounded-lg flex flex-col justify-between shadow-inner relative overflow-hidden">
                ...
                <div className="flex-1 flex flex-col items-center justify-center relative p-1 overflow-hidden">
                    {notification && (
                        <div className={`absolute top-0 z-50 px-2 py-1 rounded-full shadow-lg font-bold text-[10px] md:text-sm animate-bounce border transition-all duration-300
                    ...
                    {(phase === 'IDLE' || phase === 'MOVING') && !winner && (
                        <div className="flex flex-col items-center justify-center gap-1">
                            ...
                            <div className={`w-12 h-12 md:w-32 md:h-32 bg-white border-1 border-gray-300 rounded-lg md:rounded-3xl flex items-center justify-center shadow-lg ${isLocalRolling ? 'animate-shake' : ''}`}>
                            ...
                            {turn === myRole && (
                                <button onClick={handleRollDice} disabled={isLocalRolling} 
                                    className="px-3 py-1 md:px-8 md:py-3 rounded-full font-bold shadow-lg text-white bg-blue-600 hover:bg-blue-700 transition-all transform active:scale-95 text-[10px] md:text-base">
                    ...
                    {phase === 'INTERACTION' && interactionData && !winner && (
                        <div className="w-full max-w-full px-1 text-center animate-fadeIn overflow-y-auto max-h-full">
                            {interactionData.type === 'GROWTH' && (
                                <>
                                    <h3 className="text-xs md:text-2xl font-bold mb-1 text-purple-600">GROWTH BOOST</h3>
                                    <div className="text-xl md:text-5xl mb-1">🚀</div>
                                    <p className="text-[8px] md:text-sm font-medium text-gray-600 mb-2">Pendapatan +<span className="font-bold text-green-600">50%</span>!</p>
                                    {turn === myRole ? <button onClick={handleTakeBoost} className="w-full bg-purple-600 text-white py-1 md:py-3 rounded-lg md:rounded-xl font-bold hover:bg-purple-700 shadow-lg text-[10px] md:text-base">AMBIL</button> : <p className="text-gray-400 text-[10px]">Menunggu lawan...</p>}
                                </>
                            )}
                            {interactionData.type === 'TAX' && (
                                <>
                                    <h3 className="text-xs md:text-2xl font-bold mb-1 text-red-600">PAJAK</h3>
                                    <p className="text-lg md:text-4xl font-black text-gray-800 mb-2">- {interactionData.amount}</p>
                                    {turn === myRole ? <button onClick={handlePayTax} className="w-full bg-red-600 text-white py-1 md:py-3 rounded-lg md:rounded-xl font-bold hover:bg-red-700 shadow-lg text-[10px] md:text-base">BAYAR</button> : <p className="text-gray-400 text-[10px]">Menunggu lawan...</p>}
                                </>
                            )}
                            {interactionData.type === 'INTEREST' && (
                                <>
                                    <h3 className="text-xs md:text-2xl font-bold mb-1 text-green-600">BUNGA</h3>
                                    <p className="text-lg md:text-4xl font-black text-gray-800 mb-2">+ {interactionData.amount}</p>
                                    {turn === myRole ? <button onClick={handleClaimInterest} className="w-full bg-green-600 text-white py-1 md:py-3 rounded-lg md:rounded-xl font-bold hover:bg-green-700 shadow-lg animate-pulse text-[10px] md:text-base">KLAIM</button> : <p className="text-gray-400 text-[10px]">Menunggu lawan...</p>}
                                </>
                            )}
